...
class GC2GD:
    ...
    def detect_link_type(self, value):
        ...
        if any(host == item or host.endswith("." + item) for item in twitch_hosts):
            return "twitch"
    ...
    def fetch_torrent_file_bytes(self, source):
        ...
        headers = {
            "User-Agent": "Mozilla/5.0 GC2GD/1.2.8",
    ...
    def print_youtube_formats(self, url):
        ...
        print("If you enter only a video-only format ID, GC2GD will try to add best audio automatically.")
    ...
    def download_twitch(self, url, folder_name="", quality="best", audio_only=False, metadata=False):
        url = url.strip()
        folder_name = self.sanitize_name(folder_name)

        save_dir = self.twitch_dir / folder_name
        save_dir.mkdir(parents=True, exist_ok=True)
        ...
        if audio_only:
            ...
            selected_format = quality or "best"
    ...
    def print_help(self):
        text = """
GC2GD - Google Colab to Google Drive Downloader

Main options:
1. Auto detect link
2. Torrent tools
3. YouTube video or playlist
4. Twitch video, VOD, or clip
5. Direct link
6. Batch download
7. Download history
...
Resume support:
GC2GD uses aria2 session persistence.
Session file:
/content/drive/MyDrive/GC2GD/Logs/aria2.session

Duplicate torrent handling:
GC2GD reads torrent InfoHash before adding .torrent files.
If the same torrent already exists in aria2, it resumes or monitors the existing GID instead of adding a duplicate.
If the existing torrent is in error state, GC2GD removes it and adds the torrent again.

Google Colab may disconnect idle or long-running sessions.
GC2GD does not bypass Colab timeout.
...
Torrent seeding:
GC2GD uses 525600 minutes when seeding is enabled.
...
If you select a video-only custom format ID, GC2GD tries to add best available audio automatically.
...
def main():
    ...
    while True:
        ...
        print("4. Twitch video, VOD, or clip")
        print("5. Direct link")
